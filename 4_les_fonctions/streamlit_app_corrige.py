...
def fibonacci(n: int) -> list:
    ...
    for i in range(2, n+1):
        ...
        st.write(f"Le terme {i} de la suite de Fibonacci est : {next_fib} ({fib_sequence[i - 1]} + {fib_sequence[i - 2]})")
    return fib_sequence

# Demander à l'utilisateur combien de termes il souhaite afficher
n_terms = st.number_input("Combien de termes de la suite de Fibonacci souhaitez-vous afficher ?", min_value=1, value=10)
...
n_terms_slider = st.slider("Choisissez le nombre de termes de la suite de Fibonacci :", min_value=1, max_value=100, value=10)
