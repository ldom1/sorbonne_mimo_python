...
- [x] Il passe immédiatement à l’itération suivante de la boucle
...
- [x] Avec des blocs try-except
- [ ] Avec des blocs if-else
- [ ] Avec des blocs while
- [ ] Avec des blocs match-case

**9. Qu'est-ce qu'un array NumPy ?**
- [x] Une structure de données unidimensionnelle pour stocker des nombres
- [x] Une structure de données multidimensionnelle pour stocker des nombres
- [x] Une structure de données pour stocker des chaînes de caractères
- [x] Une structure de données pour stocker des listes

**10. Quel est l'avantage principal d'utiliser NumPy par rapport aux listes Python ?**
- [x] NumPy est plus rapide pour les opérations mathématiques et permet le calcul vectorisé
...
- [x] Une structure de données multidimensionnel basé sur NumPy
...
- [x] À afficher un graphique à l'écran
...
- [x] Filtrer ou sélectionner une partie des données d’un tableau sur la base d’une condition
...
- [x] En utilisant les annotations de type (ex: def f(x: int) -> float)
...
- [x] Un paramètre positionnel doit être passé dans l'ordre, un paramètre nommé peut être précisé par son nom lors de l'appel
...
- [x] À prédire à quelle catégorie appartient une observation (valeur discrète)
...
- [x] Un type d'apprentissage où les données ne sont pas étiquetées et l'algorithme doit trouver des structures dans les données
...
- [x] Séparer les données en ensembles d'entraînement et de test, choisir un modèle, entraîner le modèle sur l'ensemble d'entraînement, évaluer le modèle sur l'ensemble de test
...
- [x] Au nombre d'arbres dans la forêt
...
- [x] Mean Absolute Error (MAE), Mean Squared Error (MSE), R-squared
...
- [x] Un modèle qui s'adapte trop bien aux données d'entraînement et ne généralise pas bien sur de nouvelles données
...
- [x] Données structurées (ex : tables de bases de données relationnelles)
- [x] Données semi-structurées (ex : fichiers CSV, JSON, XML)
...
1. **Principe de responsabilité unique**  
   - Chaque module/fonction doit avoir une seule responsabilité.  
   - Avantages : maintenance facilitée, code plus lisible, moins d'effets de bord, testable.
   - Exemple : une fonction qui ne fait qu'une tâche (ex : calculer la moyenne d'une liste).

2. **Calcul vectorisé avec NumPy**  
   - Permet d'appliquer des opérations sur des tableaux entiers sans boucle explicite.  
   - Plus rapide que les boucles Python classiques grâce à l'implémentation en C.  
   - Exemple : `a + b` pour deux arrays NumPy.

3. **Lien entre NumPy et Pandas**  
   - Pandas utilise NumPy pour le stockage et les opérations sur les données.  
   - Exemple : filtrer un DataFrame Pandas avec un masque NumPy (`df[df['col'] > 10]`).

4. **Étapes typiques de prétraitement des données**  
   - Nettoyage (suppression/traitement des valeurs manquantes)  
   - Encodage des variables catégorielles  
   - Normalisation/standardisation  
   - Séparation des ensembles (train/test)  
   - Exemple pour chaque étape (remplacer NaN, `pd.get_dummies`, `StandardScaler`, `train_test_split`).
